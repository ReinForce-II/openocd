/***************************************************************************
 *   Copyright (C) 2018 by Reinforce-II                                    *
 *   fate@eastal.com                                                       *
 *                                                                         *
...
static char *cmsis_dap_addr;
static uint16_t cmsis_dap_port;
static bool swd_mode;

#define PACKET_SIZE (64 + 1) /* add 1 byte report number */
// #define ETH_TIMEOUT 10000
...
static const char *const info_caps_str[] = {"SWD  Supported", "JTAG Supported"};
...
static struct pending_scan_result
    pending_scan_results[MAX_PENDING_SCAN_RESULTS];
...
static uint8_t
    queued_seq_buf[1024]; /* TODO: make dynamic / move into cmsis object */

static int queued_retval;

static struct cmsis_dap *cmsis_dap_handle;

static int cmsis_dap_eth_open(void) {
    int socket_fd;
    ...
    server = gethostbyname(cmsis_dap_addr);
    if (server == NULL) {
    ...
    bcopy((char *)server->h_addr, (char *)&server_addr.sin_addr.s_addr,
          server->h_length);
    server_addr.sin_port = htons(cmsis_dap_port);
    if (connect(socket_fd, &server_addr, sizeof(server_addr)) < 0) {
        LOG_ERROR("connect failed");
        return ERROR_FAIL;
    }

    LOG_INFO("connect to %s:%u success", cmsis_dap_addr, cmsis_dap_port);

    struct cmsis_dap *dap = malloc(sizeof(struct cmsis_dap));

    dap->socket_fd = socket_fd;
    dap->caps = 0;
    dap->mode = 0;
...
static void cmsis_dap_eth_close(struct cmsis_dap *dap) {
    close(cmsis_dap_handle->socket_fd);
    cmsis_dap_handle->socket_fd = 0;
...
static int cmsis_dap_eth_xfer(struct cmsis_dap *dap, int txlen) {
    /* we should not xfer packet_buffer[0] (report number) */
#ifdef CMSIS_DAP_JTAG_DEBUG
    LOG_DEBUG("cmsis-dap xfer cmd=%02X", dap->packet_buffer[1]);
#endif
    ...
    int retval =
        send(dap->socket_fd, dap->packet_buffer + 1, dap->packet_size - 1, 0);
    if (retval == -1) {
    ...
    /* get reply */
    int bytes_read = 0;
    while (bytes_read != dap->packet_size - 1) {
        retval = recv(dap->socket_fd, dap->packet_buffer + 0 + bytes_read,
                      dap->packet_size - 1 - bytes_read, 0);
...
static int cmsis_dap_cmd_DAP_SWJ_Pins(uint8_t pins, uint8_t mask,
    ...
    retval = cmsis_dap_eth_xfer(cmsis_dap_handle, 8);
...
static int cmsis_dap_cmd_DAP_SWJ_Clock(uint32_t swj_clock) {
    ...
    retval = cmsis_dap_eth_xfer(cmsis_dap_handle, 6);
...
static int cmsis_dap_cmd_DAP_SWJ_Sequence(uint8_t s_len,
    ...
    retval = cmsis_dap_eth_xfer(cmsis_dap_handle, DIV_ROUND_UP(s_len, 8) + 3);
...
static int cmsis_dap_cmd_DAP_Info(uint8_t info, uint8_t **data) {
    ...
    buffer[2] = info;
    retval = cmsis_dap_eth_xfer(cmsis_dap_handle, 3);
...
static int cmsis_dap_cmd_DAP_LED(uint8_t leds) {
    ...
    buffer[2] = 0x00;
    buffer[3] = leds;
    retval = cmsis_dap_eth_xfer(cmsis_dap_handle, 4);
...
static int cmsis_dap_cmd_DAP_Disconnect(void) {
    ...
    retval = cmsis_dap_eth_xfer(cmsis_dap_handle, 2);
...
static int cmsis_dap_cmd_DAP_TFER_Configure(uint8_t idle, uint16_t retry_count,
    ...
    retval = cmsis_dap_eth_xfer(cmsis_dap_handle, 7);
...
static int cmsis_dap_swd_run_queue(void) {
    ...
    for (int i = 0; i < pending_transfer_count; i++) {
        ...
        LOG_DEBUG_IO("%s %s reg %x %" PRIx32, cmd & SWD_CMD_APnDP ? "AP" : "DP",
         ...
         * cmsis_dap_init().
         */
        if (!(cmd & SWD_CMD_RnW) && !(cmd & SWD_CMD_APnDP) &&
            (cmd & SWD_CMD_A32) >> 1 == DP_CTRL_STAT && (data & CORUNDETECT)) {
    ...
    queued_retval = cmsis_dap_eth_xfer(cmsis_dap_handle, idx);
    if (queued_retval != ERROR_OK)
        goto skip;
...
static void cmsis_dap_swd_write_reg(uint8_t cmd, uint32_t value,
                                    uint32_t ap_delay_clk) {
    assert(!(cmd & SWD_CMD_RnW));
    cmsis_dap_swd_queue_cmd(cmd, NULL, value);
}

static void cmsis_dap_swd_read_reg(uint8_t cmd, uint32_t *value,
                                   uint32_t ap_delay_clk) {
    assert(cmd & SWD_CMD_RnW);
...
static int cmsis_dap_get_status(void) {
    ...
    if (retval == ERROR_OK) {
        LOG_INFO("SWCLK/TCK = %d SWDIO/TMS = %d TDI = %d TDO = %d nTRST = %d "
                 "nRESET = %d",
                 (d & SWJ_PIN_TCK) ? 1 : 0, (d & SWJ_PIN_TMS) ? 1 : 0,
                 (d & SWJ_PIN_TDI) ? 1 : 0, (d & SWJ_PIN_TDO) ? 1 : 0,
                 (d & SWJ_PIN_TRST) ? 1 : 0, (d & SWJ_PIN_SRST) ? 1 : 0);
    }
...
static int cmsis_dap_swd_switch_seq(enum swd_special_seq seq) {
    ...
    retval = cmsis_dap_cmd_DAP_Connect(
        seq == LINE_RESET || seq == JTAG_TO_SWD ? CONNECT_SWD : CONNECT_JTAG);
    if (retval != ERROR_OK)
        return retval;
...
static int cmsis_dap_init(void) {
    ...
    if (data[0] == 2) { /* short */
        ...
        pending_transfers =
            malloc(pending_queue_len * sizeof(*pending_transfers));
        ...
        if (cmsis_dap_handle->packet_size != pkt_sz + 1) {
            ...
            cmsis_dap_handle->packet_buffer = realloc(
                cmsis_dap_handle->packet_buffer, cmsis_dap_handle->packet_size);
...
static int cmsis_dap_quit(void) {
    ...
    cmsis_dap_eth_close(cmsis_dap_handle);
...
static void cmsis_dap_execute_reset(struct jtag_command *cmd) {
    ...
    int retval =
        cmsis_dap_cmd_DAP_SWJ_Pins(pins, SWJ_PIN_TRST | SWJ_PIN_SRST, 0, NULL);
    if (retval != ERROR_OK)
...
/* Set TMS high for five TCK clocks, to move the TAP to the Test-Logic-Reset
 * state */
...
static void debug_parse_cmsis_buf(const uint8_t *cmd, int cmdlen) {
    /* cmd is a eth packet to go to the cmsis-dap interface */
    ...
    case CMD_DAP_JTAG_SEQ: {
        ...
        for (int seq = 0; seq < cmd[2]; ++seq) {
            ...
            if (len == 0)
                len = 64;
            printf("  sequence %d starting %d: info %02x (len=%d tms=%d "
                   "read_tdo=%d): ",
                   seq, pos, info, len, info & DAP_JTAG_SEQ_TMS,
                   info & DAP_JTAG_SEQ_TDO);
        ...
        if (pos != cmdlen) {
            printf("BUFFER LENGTH MISMATCH looks like %d but %d specified", pos,
                   cmdlen);
            exit(-1);
        }
...
static void cmsis_dap_flush(void) {
    ...
    DEBUG_JTAG_IO("Flushing %d queued sequences (%d bytes) with %d pending "
                  "scan results to capture",
                  queued_seq_count, queued_seq_buf_end,
    ...
    int retval = cmsis_dap_eth_xfer(cmsis_dap_handle, queued_seq_buf_end + 3);
    if (retval != ERROR_OK || buffer[1] != DAP_OK) {
    ...
    for (int i = 0; i < pending_scan_result_count; ++i) {
        ...
        DEBUG_JTAG_IO("Copying pending_scan_result %d/%d: %d bits from byte %d "
                      "-> buffer + %d bits",
                      i, pending_scan_result_count, scan->length,
                      scan->first + 2, scan->buffer_offset);
#ifdef CMSIS_DAP_JTAG_DEBUG
        for (uint32_t b = 0; b < DIV_ROUND_UP(scan->length, 8); ++b)
            printf("%02X ", buffer[2 + scan->first + b]);
        printf("\n");
#endif
        bit_copy(scan->buffer, scan->buffer_offset, buffer + 2 + scan->first, 0,
                 scan->length);
    }
...
/* queue a sequence of bits to clock out TDI / in TDO, executing if the buffer
 * is full.
 *
...
static void cmsis_dap_add_jtag_sequence(int s_len, const uint8_t *sequence,
                                        int s_offset, bool tms,
                                        uint8_t *tdo_buffer,
                                        int tdo_buffer_offset) {
    DEBUG_JTAG_IO(
        "[at %d] %d bits, tms %s, seq offset %d, tdo buf %p, tdo offset %d",
        queued_seq_buf_end, s_len, tms ? "HIGH" : "LOW", s_offset, tdo_buffer,
        tdo_buffer_offset);
    ...
    if (s_len > 64) {
        ...
        for (int offset = 0; offset < s_len; offset += 64) {
            ...
            if (len > 64)
                len = 64;
            DEBUG_JTAG_IO("Splitting long jtag sequence: %d-bit chunk starting "
                          "at offset %d",
                          len, offset);
            cmsis_dap_add_jtag_sequence(
                len, sequence, s_offset + offset, tms, tdo_buffer,
    ...
    if (queued_seq_count >= 255 ||
        queued_seq_buf_end + cmd_len > QUEUED_SEQ_BUF_LEN)
    ...
    queued_seq_buf[queued_seq_buf_end] =
        ...
        (tdo_buffer != NULL ? DAP_JTAG_SEQ_TDO : 0) | (s_len == 64 ? 0 : s_len);

    if (sequence != NULL)
        bit_copy(&queued_seq_buf[queued_seq_buf_end + 1], 0, sequence, s_offset,
                 s_len);
    else
        memset(&queued_seq_buf[queued_seq_buf_end + 1], 0,
               DIV_ROUND_UP(s_len, 8));

    queued_seq_buf_end += cmd_len;

    if (tdo_buffer != NULL) {
        struct pending_scan_result *scan =
            &pending_scan_results[pending_scan_result_count++];
...
static void cmsis_dap_state_move(void) {
    ...
    DEBUG_JTAG_IO("state move from %s to %s: %d clocks, %02X on tms",
                  ...
                  tap_state_name(tap_get_end_state()), tms_scan_bits, tms_scan);
...
static void cmsis_dap_execute_scan(struct jtag_command *cmd) {
    ...
    for (int i = 0; i < cmd->cmd.scan->num_fields; i++, field++) {
        ...
        DEBUG_JTAG_IO("%s%s field %d/%d %d bits", field->in_value ? "in" : "",
                      field->out_value ? "out" : "", i,
                      cmd->cmd.scan->num_fields, field->num_bits);
        ...
        if (i == cmd->cmd.scan->num_fields - 1 &&
            ...
            /* Last field, and we're leaving IRSHIFT/DRSHIFT. Clock last bit
             * during tap movement. This last field can't have length zero, it
             * was checked above. */
            cmsis_dap_add_jtag_sequence(field->num_bits -
            ...
            uint8_t last_bit = 0;
            if (field->out_value)
                bit_copy(&last_bit, 0, field->out_value, field->num_bits - 1,
                         1);
            cmsis_dap_add_jtag_sequence(1, &last_bit, 0, true, field->in_value,
                                        field->num_bits - 1);
            tap_set_state(tap_state_transition(tap_get_state(), 1));

            /* Now clock one more cycle, with TMS low, to get us into a PAUSE
             * state */
            cmsis_dap_add_jtag_sequence(1, &last_bit, 0, false, NULL, 0);
            ...
            cmsis_dap_add_jtag_sequence(field->num_bits, field->out_value, 0,
                                        false, field->in_value, 0);
        }
    }
...
COMMAND_HANDLER(cmsis_dap_handle_addr_command) {
    if (CMD_ARGC == 1) {
        cmsis_dap_addr = calloc(strlen(CMD_ARGV[0]) + 1, sizeof(char));
        if (cmsis_dap_addr == NULL) {
            LOG_ERROR("unable to allocate memory");
            return ERROR_OK;
        }
        strcpy(cmsis_dap_addr, CMD_ARGV[0]);
    } else {
        LOG_ERROR("expected exactly one argument to cmsis_dap_addr "
                  "<address>");
    }
...
COMMAND_HANDLER(cmsis_dap_handle_port_command) {
    if (CMD_ARGC == 1) {
        int port = atoi(CMD_ARGV[0]);
        ...
        LOG_ERROR("expected exactly one argument to cmsis_dap_port "
                  "<port-number>");
    }
...
static const struct command_registration cmsis_dap_command_handlers[] = {
    ...
    {
        .name = "cmsis_dap_addr",
        .handler = &cmsis_dap_handle_addr_command,
        .mode = COMMAND_CONFIG,
        .help = "set the address of the adapter",
        .usage = "addr_string",
    },
    {
        .name = "cmsis_dap_port",
        .handler = &cmsis_dap_handle_port_command,
        .mode = COMMAND_CONFIG,
        .help = "set the port number of the adapter",
        .usage = "port_string",
    },
    COMMAND_REGISTRATION_DONE};
...
struct jtag_interface cmsis_dap_eth_interface = {
    .name = "cmsis-dap-eth",
